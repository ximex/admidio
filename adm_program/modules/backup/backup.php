...
if($getMode == 'show_list')
{
    ...
    $backupMenu->addItem('admMenuItemNewBackup', $g_root_path.'/adm_program/modules/backup/backup.php?mode=create_backup',
                         $gL10n->get('BAC_START_BACKUP'), 'cloud-download');
    ...
    foreach($existingBackupFiles as $key => $old_backup_file)
    {
        ...
        $columnValues = array(
            '<a href="'.$g_root_path.'/adm_program/modules/backup/backup_file_function.php?job=get_file&amp;filename='. $old_backup_file. '">
                <span class="fa fa-file-zip-o fa-gap-right" aria-hidden="true"></span>'.$old_backup_file.
            '</a>',
            date($gPreferences['system_date'].' '.$gPreferences['system_time'],
            filemtime($backupabsolutepath.$old_backup_file)),
            ...
            '<a class="admidio-icon-link" data-toggle="modal" data-target="#admidio_modal"
                ...
                $key.'&amp;name='.urlencode($old_backup_file).'&amp;database_id='.$old_backup_file.'">
...
elseif($getMode == 'create_backup')
{
    ...
    $form->addSubmitButton('btn_update_overview', $gL10n->get('BAC_BACK_TO_BACKUP_PAGE'), array('icon' => 'arrow-circle-left'));
